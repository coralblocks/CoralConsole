...
check_prerequisites() {
  ...
  for required_file in Dockerfile Dockerfile.dev docker-compose.yml docker-compose.dev.yml package.json package-lock.json scripts/docker-common.sh scripts/change-config.sh; do
...
prompt_value() {
  label=$1
  default_value=$2
  printf '%s [%s] (you can change it later by editing the .env file or running ./scripts/change-config.sh): ' "$label" "$default_value" >&2
...
choose_allowed_clients() {
  ...
  if ! prompt_yes_no "Configure an IP/CIDR client allowlist (you can change it later by editing the .env file or running ./scripts/change-config.sh)"; then
    return 0
  fi
...
choose_access_key() {
  ...
  if ! prompt_yes_no "Generate a random shared access key (you can change it later by editing the .env file or running ./scripts/change-config.sh)"; then
    return 0
  fi
