...
- Keep Add Actor, List Actors, and Refresh Now together in that order at the upper right of the Actor Map, using the same lightweight ghost-button treatment. List Actors opens `/actors` in a new tab with no back link; its columns are Name, Type, Class, REST IP, REST PORT, Online?, Edit, and Remove. Endpoint edits, removals, and row ordering persist in SQLite, endpoint edits close the actor’s old monitoring connection, no-op drags do not save, and removals require confirmation.
