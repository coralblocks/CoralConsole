...
- `localStorage` is device-specific only: `coral-console-intro` stores intro visibility and `coral-console-counts` stores Actor Counts panel visibility. `coral-console-actors` is a legacy import source and must not become the canonical topology again.
