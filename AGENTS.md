...
- Actor Details places a dedicated recent-log panel above the separate REST Admin panel. The server polls `VM lastLogs` with `shouldLog: false`, and the browser fetches `/api/actors/<id>/logs` at the configured actor interval. Render the returned messages as bounded plain monospace text without interpreting SGR sequences for now.
