...
- `localStorage` is device-specific only: `coral-console-intro` stores intro visibility, `coral-console-counts` stores Actor Counts panel visibility, and `coral-console-actor-card-widths` stores the browser's Actor Map card-width preference. `coral-console-actors` is a legacy import source and must not become the canonical topology again.
