...
- Probe the minimal same-origin `/api/health` endpoint every five seconds from the open home page, independently of actor polling. If the request fails, times out, or returns a non-success response, immediately mark the Actor Map as stale with an obvious light-coral treatment and warning; remove the warning automatically after the endpoint succeeds again.
