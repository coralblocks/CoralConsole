...
- Every actor card is concise and type-independent: icon, actor/account name, sequence immediately to the name's right, class, and operational-state badge only. Do not show an Online/Offline dot on topology cards; preserve connectivity in the accessible card label. Never add endpoint, session, accounts, clock tick, signal, or footer metadata below the icon.
...
- Every actor detail header shows its operational-state badge immediately to the left of its separate Online/Offline connectivity badge. Give Active, Inactive, Closed, Rewinding, and Disconnected distinct cyan, neutral gray, coral, violet, and amber treatments respectively. Keep the connectivity badge on this page, and apply the same diagonal coral-red stripe pattern to the full detail header when the actor is Offline.
