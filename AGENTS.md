...
- Recent actor log windows and their `messagesSaved` cursors are process-memory cache only. They are intentionally not written to SQLite and may be repopulated after an application restart.
...
- System Pulse count labels use uppercase `ONLINE` and `OFFLINE`. Keep Actor Map's `Refresh Now` control in the topology header's upper-right corner, above the filter row. It immediately polls `actorStatus` and actor-account `list` for every actor, bypassing the normal interval without running `healthCheck` or creating audit rows.
