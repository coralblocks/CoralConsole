...
- Actor Details REST Admin provides an Admin account selector containing the persisted actor account and `VM`, followed by an Admin action selector populated alphabetically from the selected account's persisted action list without rewriting its stored order. Changing the account immediately selects from that account's actions. Manual actions are audited against the actor row used to reach the shared REST endpoint.
