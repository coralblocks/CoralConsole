...
The optional `shouldLog` boolean defaults to `true` at the actor. Omit it for discovery and operator-triggered admin actions, including manually selected `healthCheck`. Server-driven monitoring requests over the persistent actor connection—`actorStatus`, scoped `list`, and `VM lastLogs`, including post-action reconciliation—must send `"shouldLog": false` so routine polling does not fill actor logs.
