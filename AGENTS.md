...
- Treat desktop and laptop browsers as the primary targets. Keep the conventional desktop layout through 1999 CSS pixels, then use the coordinated large-desktop tier at 2000 CSS pixels and above so content width, typography, controls, cards, tables, and spacing grow together on 2560–3008+ pixel viewports. Mobile remains a functional fallback, not the principal design target; do not use iframe, page transforms, or blanket browser zoom for responsive sizing.
