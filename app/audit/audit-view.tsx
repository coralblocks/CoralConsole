...
export default function AuditView() {
  ...
  async function clearHistory() {
    if (!window.confirm("Permanently clear the command audit history for this installation?")) return;
    try {
  ...
  return (
    <main className="console-shell audit-page">
      <header className="topbar">
        <Link className="brand" href="/" aria-label="CoralConsole topology"><span className="brand-mark" aria-hidden="true"><i /><i /><i /></span><span><strong>CoralConsole</strong><small>Command audit</small></span></Link>
        <Link className="button button-ghost detail-back" href="/">← Back to topology</Link>
      </header>
      <section className="audit-main">
        <div className="audit-title"><div><p className="eyebrow">Shared operations record</p><h1>Command audit</h1><p>Inputs, outputs, outcomes, and timing for administrative actions run through this CoralConsole installation.</p></div><button className="remove-button standalone" type="button" onClick={() => void clearHistory()} disabled={!entries.length}>Clear audit history</button></div>

        <form className="audit-controls" onSubmit={search}>
          <label htmlFor="audit-search">Search audit history</label>
          <div><input id="audit-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Actor, endpoint, command, or output" /><button className="button button-dark" type="submit">Search</button></div>
        ...
        <section className="audit-table-wrap" aria-live="polite">
          {loading ? <p className="empty-audit">Loading audit history…</p> : !entries.length ? <p className="empty-audit">No commands match these filters.</p> : (
            <table className="audit-table">
              <thead><tr><th>Time</th><th>Actor</th><th>Command</th><th>Parameters</th><th>Outcome</th><th>Duration</th><th>Output</th></tr></thead>
