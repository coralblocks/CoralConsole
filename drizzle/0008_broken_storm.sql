...
CREATE TABLE `__new_actors` (
	...
	`operational_state` text DEFAULT 'inactive' NOT NULL,
	`host` text NOT NULL,
	...
	`outbound_sequence` text DEFAULT 'Not reported' NOT NULL,
	`accounts` text DEFAULT 'Not reported' NOT NULL,
	`clock_tick_interval` text DEFAULT 'Not reported' NOT NULL,
	`session_started` text,
	`status_responded_at` text,
...
INSERT INTO `__new_actors`("id", "name", "kind", "status", "host", "port", "account", "class_name", "cluster", "sequencer_role", "latency", "session", "outbound_sequence", "accounts", "clock_tick_interval", "session_started", "status_responded_at", "last_seen", "last_seen_at", "last_error", "commands", "demo", "created_at", "updated_at")
SELECT
	"id",
	"name",
	"kind",
	CASE WHEN "status" IN ('healthy', 'online', 'standby') THEN 'online' ELSE 'offline' END,
	"host",
	"port",
	"account",
	"class_name",
	...
	"created_at",
	"updated_at"
FROM `actors`;--> statement-breakpoint
